...
while(<IN>){
	...
	my @print_line = ($SampleID, $SampleName, $Sample, $Geno, $Total, $Mutation, $Ratio, $cffDNA , $Pvalue);
	@print_line = ( @print_line, $Mother_E, $Fetal_E, $Father_E );
...
foreach my$S(keys %Sample_Fetal_Geno){
	...
	foreach my$G( @Fetal_Geno ){
		if( not exists $Ratio_high{$S}{$G} and $G ne "N" ){
...
sub Geno_JianYan{
	...
	if( exists $people_geno->{$sample} ){
		...
		if( exists $people_geno->{$sample}->{"2"} ){
			...
			#@{$Sample_Fetal_Geno{$SampleName}} = @Fetal_Geno;
		}
